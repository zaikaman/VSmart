          ...
          <CardContent className="flex items-start gap-4 p-6">
            ...
            <div>
              ...
              <p className="mt-2 text-sm text-slate-600">
                Hàng chờ duyệt chỉ hiển thị cho tài khoản quản trị hoặc quản lý để tránh nhầm luồng thao tác.
              </p>
            </div>
          </CardContent>
        </Card>
  ...
  return (
    <div className="container mx-auto max-w-7xl px-6 py-8">
      <div className="mb-8 flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
        <div>
          ...
          <p className="mt-2 max-w-2xl text-sm text-slate-500">
            Duyệt nhanh các task đã hoàn thành, gửi lại nhận xét rõ ràng và giữ lịch sử ra quyết định tập trung ở một chỗ.
          </p>
        </div>
        ...
        <div className="flex gap-2">
          ...
          <div className="grid gap-3 sm:grid-cols-3">
            <Card className="min-w-[140px] border-[#e7ebdf]">
              <CardContent className="p-4">
                <p className="text-xs uppercase tracking-[0.18em] text-slate-500">Đang chờ</p>
                <p className="mt-2 text-2xl font-semibold text-slate-900">{queueStats.total}</p>
              </CardContent>
            </Card>
            <Card className="min-w-[140px] border-[#ffe2d4] bg-[#fff7f2]">
              <CardContent className="p-4">
                <p className="text-xs uppercase tracking-[0.18em] text-slate-500">Ưu tiên cao</p>
                <p className="mt-2 text-2xl font-semibold text-[#f97316]">{queueStats.highPriority}</p>
              </CardContent>
            </Card>
            <Card className="min-w-[140px] border-[#ffe1e4] bg-[#fff5f6]">
              <CardContent className="p-4">
                <p className="text-xs uppercase tracking-[0.18em] text-slate-500">Quá hạn</p>
                <p className="mt-2 text-2xl font-semibold text-[#e11d48]">{queueStats.overdue}</p>
              </CardContent>
            </Card>
          </div>
        </div>
        ...
        <Card className="border-dashed border-[#dbe4ce] bg-[#f8fbf3]">
          <CardContent className="py-14 text-center">
            ...
            <p className="mx-auto mt-2 max-w-lg text-sm text-slate-500">
              Hàng chờ đang trống. Khi đội gửi task sang bước phê duyệt, bạn sẽ thấy chúng xuất hiện ở đây kèm deadline và ghi chú liên quan.
            </p>
          </CardContent>
        </Card>
      ) : (
                  ...
                  <div className="flex items-start justify-between gap-4">
                    <div>
                      ...
                      <p className="mt-2 text-sm text-slate-500">
                        {task.phan_du_an?.du_an?.ten || 'Dự án'} • {task.phan_du_an?.ten || 'Phần dự án'}
                      </p>
                    </div>
                ...
                <CardContent className="space-y-4 p-5">
                  <div className="grid gap-3 sm:grid-cols-3">
                    <div className="rounded-2xl bg-slate-50 p-3">
                      <p className="text-xs uppercase tracking-[0.18em] text-slate-500">Người phụ trách</p>
                      <p className="mt-2 text-sm font-medium text-slate-900">{task.nguoi_dung?.ten || 'Chưa phân công'}</p>
                    </div>
                    <div className="rounded-2xl bg-slate-50 p-3">
                      <p className="text-xs uppercase tracking-[0.18em] text-slate-500">Tiến độ</p>
                      <p className="mt-2 text-sm font-medium text-slate-900">
                        {task.progress_mode === 'checklist' ? `${effectiveProgress}%` : progressLabel}
                      </p>
                    </div>
                    <div className="rounded-2xl bg-slate-50 p-3">
                      <p className="text-xs uppercase tracking-[0.18em] text-slate-500">Mức ưu tiên</p>
                      <p className="mt-2 text-sm font-medium capitalize text-slate-900">{task.priority}</p>
                    </div>
                  </div>

                  {task.mo_ta ? (
                    <p className="line-clamp-3 text-sm text-slate-600">{task.mo_ta}</p>
                  ) : (
                    <p className="text-sm text-slate-400">Task này chưa có mô tả chi tiết.</p>
                  )}
                  ...
                  <div className="flex flex-wrap gap-2">
                    ...
                    <Button
                      ...
                      disabled={approveTask.isPending || rejectTask.isPending}
                    >
                      {rejectTask.isPending && rejectTask.variables?.taskId === task.id ? (
                        <Loader2 className="mr-2 h-4 w-4 animate-spin" />
                      ) : (
      ...
      <ShortcutDialog
        ...
        items={[{ keyLabel: '?', description: 'Mở bảng phím tắt của hàng chờ duyệt' }]}
      />
    </div>
  );
}
