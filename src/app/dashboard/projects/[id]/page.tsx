...
import { AlertCircle, ArrowLeft, CheckCircle, Clock, Loader2, Pencil, Plus, Trash2, Users } from 'lucide-react';
...
import { useDeleteProjectPart, useProjectParts, type ProjectPart } from '@/lib/hooks/use-project-parts';
...
export default function ProjectDetailPage() {
    ...
    return (
      <div className="container mx-auto p-6">
        <div className="py-12 text-center">
          ...
          <p className="mb-4 text-gray-500">Dự án này không tồn tại hoặc bạn không còn quyền truy cập.</p>
  ...
  return (
    <div className="container mx-auto p-6">
      <Link href="/dashboard/projects" className="mb-6 inline-flex items-center text-sm text-slate-500 hover:text-slate-900">
      ...
      <div className="mb-8 flex items-start justify-between gap-4">
        <div>
          <div className="mb-2 flex items-center gap-3">
            ...
            {project.current_membership_role ? <Badge variant="secondary">Vai trò: {project.current_membership_role}</Badge> : null}
          </div>
          {project.mo_ta ? <p className="max-w-2xl text-gray-600">{project.mo_ta}</p> : null}
        </div>
        {canManageProject ? (
          <Button onClick={() => setCreatePartOpen(true)} className="bg-black text-white hover:bg-gray-800">
      ...
      {forecast && forecast.forecastStatus !== 'on-track' ? (
        <div className={`mb-8 rounded-2xl border p-5 ${forecast.forecastStatus === 'slipping' ? 'border-rose-200 bg-rose-50' : 'border-amber-200 bg-amber-50'}`}>
          <div className="flex flex-col gap-4 md:flex-row md:items-start md:justify-between">
            <div>
              <p className="text-xs font-semibold uppercase tracking-[0.18em] text-slate-500">Forecast dự án</p>
              <h2 className="mt-2 text-xl font-semibold text-slate-900">
                {forecast.forecastStatus === 'slipping' ? `Nguy cơ trễ khoảng ${forecast.projectedDelayDays} ngày` : 'Dự án bắt đầu có tín hiệu cần theo dõi'}
              </h2>
      ...
      <div className="mb-8 grid gap-4 md:grid-cols-3">
        <Card>
          ...
          <CardContent>
            ...
            <div className="mt-2 h-2 w-full rounded-full bg-gray-100">
              <div className="h-2 rounded-full bg-[#b9ff66]" style={{ width: `${project.phan_tram_hoan_thanh || 0}%` }} />
            </div>
          </CardContent>
        </Card>
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold">{new Date(project.deadline).toLocaleDateString('vi-VN')}</div>
      ...
      <div className="grid gap-6 xl:grid-cols-[1.2fr_0.8fr]">
                  ...
                  <CardHeader>
                    <div className="flex items-start justify-between">
                      <div className="flex-1">
                        <CardTitle className="text-lg transition-colors group-hover:text-[#8abe4b]">{part.ten}</CardTitle>
                      </div>
                      <Badge variant="outline" className={`${statusColors[part.trang_thai]} border`}>
                        {statusLabels[part.trang_thai]}
                      </Badge>
                    </div>
                  </CardHeader>
                  <CardContent>
                    {part.mo_ta ? <p className="mb-3 line-clamp-2 text-sm text-gray-600">{part.mo_ta}</p> : null}
                    <div className="mb-2 flex items-center justify-between text-sm">
                      <span className="text-gray-500">Tiến độ: <span className="font-medium">{part.phan_tram_hoan_thanh?.toFixed(0) || 0}%</span></span>
                      <span className="text-gray-500">Deadline: <span className="font-medium">{new Date(part.deadline).toLocaleDateString('vi-VN')}</span></span>
                    </div>
                    <div className="mb-3 h-1.5 w-full rounded-full bg-gray-100">
                      <div className="h-1.5 rounded-full bg-[#b9ff66]" style={{ width: `${part.phan_tram_hoan_thanh || 0}%` }} />
                    </div>
                    {canManageProject ? (
                      <div className="flex gap-2">
                        ...
                        <Button size="sm" variant="outline" className="flex-1 text-red-600 hover:bg-red-50 hover:text-red-700" onClick={() => setDeletePartId(part.id)}>
        ...
        <div className="space-y-6">
          <Card>
            <CardHeader>
              <CardTitle className="flex items-center"><Users className="mr-2 h-5 w-5" /> Quản lý thành viên</CardTitle>
            </CardHeader>
            <CardContent>
      ...
      <CreatePartModal open={createPartOpen} onOpenChange={setCreatePartOpen} projectId={projectId} />

      {selectedPart ? (
        <EditPartModal open={editPartOpen} onOpenChange={setEditPartOpen} projectId={projectId} part={selectedPart} />
      ) : null}
      ...
      <AlertDialog open={!!deletePartId} onOpenChange={() => setDeletePartId(null)}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>Bạn có chắc muốn xóa phần dự án này? Hành động này không thể hoàn tác.</AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
            <AlertDialogCancel>Hủy</AlertDialogCancel>
            <AlertDialogAction onClick={() => deletePartId && handleDeletePart(deletePartId)} className="bg-red-600 hover:bg-red-700" disabled={deletePartMutation.isPending}>
              {deletePartMutation.isPending ? <><Loader2 className="mr-2 h-4 w-4 animate-spin" /> Đang xóa...</> : 'Xóa'}
            </AlertDialogAction>
          </AlertDialogFooter>
        </AlertDialogContent>
      </AlertDialog>
