...
import { useInsightFeedback, useRebalanceSuggestions } from '@/lib/hooks/use-ai-insights';
...
export function RebalancePanel({
  projectId,
  title = 'AI cân lại tải công việc',
  compact = false,
}: RebalancePanelProps) {
      ...
      <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
        <div>
          ...
          <p className="mt-1 text-sm text-[#61705f]">
            Gợi ý điều phối lại task khi đội bắt đầu dồn tải hoặc có deadline sát nhau.
          </p>
        </div>
        ...
        <div className="mt-5 space-y-4">
          ...
          {result.result.suggestions.length === 0 ? (
            <div className="rounded-2xl border border-dashed border-[#d8e0ce] px-4 py-5 text-sm text-[#61705f]">
              Hiện chưa thấy phương án điều phối nào đủ tốt để gợi ý.
            </div>
          ) : (
            <div className="space-y-3">
              {result.result.suggestions.map((item) => (
                <div key={`${item.task_id}-${item.to_user_id}`} className="rounded-2xl border border-[#ecefe6] bg-[#fbfbf8] p-4">
