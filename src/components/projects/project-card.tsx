    ...
    <Link href={`/dashboard/projects/${project.id}`}>
      <div className="border border-gray-100 rounded-[20px] p-5 hover:shadow-xl transition-all cursor-pointer bg-white group hover:border-[#b9ff66]/50">
        <div className="flex items-start justify-between mb-3">
          <h3 className="font-bold text-lg group-hover:text-[#191a23] transition-colors line-clamp-2">{project.ten}</h3>
          <Badge className={`${statusColors[project.trang_thai as keyof typeof statusColors]} border-0 px-3 py-1 rounded-full shrink-0`}>
