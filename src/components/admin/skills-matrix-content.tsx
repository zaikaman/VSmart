...
export function SkillsMatrixPageContent() {
  const { data, isLoading, error } = useQuery<SkillsMatrixResponse>({
    ...
    staleTime: 5 * 60 * 1000, // 5 phút - skills matrix ít thay đổi
    ...
    <div className="container mx-auto p-6 max-w-6xl">
      <div className="flex items-center gap-4 mb-6">
        ...
        <div>
          ...
          <p className="text-muted-foreground mt-1">
            Tổng quan về kỹ năng của các thành viên trong tổ chức
          </p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
        <Card>
          <CardContent className="pt-6">
            <div className="text-center">
              <p className="text-3xl font-bold text-primary">{data.data.tong_nguoi_dung}</p>
              <p className="text-sm text-muted-foreground mt-1">Tổng số thành viên</p>
            </div>
          </CardContent>
        </Card>
        ...
        <Card>
          <CardContent className="pt-6">
            <div className="text-center">
              <p className="text-3xl font-bold text-primary">{data.data.tong_ky_nang}</p>
              <p className="text-sm text-muted-foreground mt-1">Tổng số kỹ năng</p>
            </div>
          </CardContent>
        </Card>
            ...
            <div className="text-center">
              <p className="text-3xl font-bold text-primary">
                {data.data.skills.length > 0
                  ? (data.data.skills.reduce((acc, s) => acc + s.tong_nam_kinh_nghiem, 0) / data.data.skills.length).toFixed(1)
                  : 0}
              </p>
              <p className="text-sm text-muted-foreground mt-1">TB năm kinh nghiệm</p>
            </div>
          </CardContent>
        </Card>
      </div>
